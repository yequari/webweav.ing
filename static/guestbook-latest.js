...
class GuestbookForm extends HTMLElement {
    ...
    constructor() {
        ...
        this._noemail = this.hasAttribute('noemail');
        this._nosite = this.hasAttribute('nosite');
    ...
    render() {
      ...
      <form action="${this._postUrl}" method="post">
        ...
        <p>Guestbook powered by <a href="https://webweav.ing">webweav.ing</a>.</p>
        <div>
                ...
                : this.error
                    ...
                    : this.comments.length === 0
                        ...
                        : this.comments.map(comment => {
                            let authorHtml = `<span class="author">${comment.AuthorName || 'Unknown Author'}</span>`
                            if (comment.AuthorSite) {
                                authorHtml = `<span class="author"><a href="${comment.AuthorSite}" target="_blank">${comment.AuthorName || 'Unknown Author'}</a></span>`
                            }
                            return `
                  <div class="comment">
                    ${authorHtml}
